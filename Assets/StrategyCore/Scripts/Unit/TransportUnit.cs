    ...
    public class TransportUnit : MonoBehaviour
    {
        ...
        public void Embark(Unit unitToTake)
        {
            if (unitToTake != null)
            {
                ...
                {
                    UIManager.instance.ShowNotifyMsg("Not enough space!", thisUnit.owner, true);
                }
                else if (unitToTake.stunned || unitToTake.muted || unitToTake.disarmed || unitToTake.polymorphed)
                {
                    UIManager.instance.ShowNotifyMsg("This unit can not embark currently!", thisUnit.owner, true);
                }
                else
                {
        ...
        public void Disembark()
        {
            ...
            if (terrainType == -1)
            {
                UIManager.instance.ShowNotifyMsg("Can not unload at this location!", thisUnit.owner, true);
                return;
            }
