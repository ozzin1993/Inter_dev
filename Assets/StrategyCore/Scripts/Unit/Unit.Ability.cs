    ...
    public partial class Unit
    {
        ...
        public bool IsCooldownGood(int abilityIndex, bool isItem)
        {
            for (int i = 0; i < cooldownAbilityIndex.Count; i++)
            {
                ...
                {
                    if (this == PlayerControl.instance.activeUnit && (SlotManager.instance.debugMode || owner == SlotManager.instance.currentPlayer)) UIManager.instance.ShowNotifyMsg("Wait until the cooldown is over.");
                    return false;
                }
            }
            return true;
        ...
        private bool CheckAbilityItemRequirements(int player, int abilityIndex, bool isItem, Ability currentAbility = null)
        {
            if (isItem)
            {
                ...
                {
                    UIManager.instance.ShowNotifyMsg("Not enough mana", owner, true);
                    return true;
                }
                ...
                //         {
                //             UIManager.instance.ShowNotifyMsg("Not enough " + items[abilityIndex].cost[0].data[i].type.name);
                //             return true;
                //         }
                //     }
                // }
            ...
            else
            {
                ...
                if (currentAbility.cost.Length > abilityLevel[abilityIndex])
                {
                    for (int i = 0; i < currentAbility.cost[abilityLevel[abilityIndex]].data.Length; i++)
                    {
                        ...
                        {
                            UIManager.instance.ShowNotifyMsg("Not enough " + currentAbility.cost[abilityLevel[abilityIndex]].data[i].type.displayName, owner, true);
                            return true;
                        }
                    }
                }
