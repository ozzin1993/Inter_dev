...
namespace StrategyCore
{
    ...
    public partial class UIManager : MonoBehaviour
    {
        ...
        [Tooltip("Имя панели-сетки умений (Center, rows=2, cols=3): верхний ряд — умения ГЗ, нижний — умения Героя.")]
        ...
        /// <summary>
        /// Наполняет панель «Умения» (2×3): верхний ряд (0..2) — умения ГЗ позиционно, нижний ряд (3..5) —
        /// умения Героя (пока пусто — шаг 5). Позиционное закрепление: скрытое/отсутствующее умение → пустой
        /// слот на своей позиции (соседи не сдвигаются).
        /// </summary>
        public void RefreshAbilitySlots()
        {
            ...
            List<BottomTableButton> slots = new List<BottomTableButton>();
            slots.AddRange(BuildCentralAbilitySlots(team, 3));   // верхний ряд — умения ГЗ
            slots.AddRange(BuildHeroAbilitySlots(team, 3));      // нижний ряд — умения Героя

            SetSlotPanelButtons(panel, slots);
        }

        // Слоты умений ГЗ позиционно: полный набор centralAbilities (позиции), видимое → кнопка с кастом,
        // скрытое/пустое → null (пустой слот). Каст переиспользует ActivateAbilityCell (серверо-авторитетно).
        List<BottomTableButton> BuildCentralAbilitySlots(int team, int count)
        {
            List<BottomTableButton> res = new List<BottomTableButton>();
            MatchManager mm = MatchManager.instance;
            TeamWaveConfig cfg = mm != null ? mm.Team(team) : null;
            IReadOnlyList<Ability> all = cfg != null ? cfg.centralAbilities : null;
            IReadOnlyList<Ability> visible = mm != null ? mm.VisibleCentralAbilities(team) : null;

            for (int i = 0; i < count; i++)
            {
                Ability ab = (all != null && i < all.Count) ? all[i] : null;
                if (ab != null && AbilityIsVisible(ab, visible))
                {
                ...
                {
                    res.Add(null);   // пустой слот на своей позиции
                }
            }
            return res;
        }

        static bool AbilityIsVisible(Ability ability, IReadOnlyList<Ability> visible)
        {
            if (ability == null || visible == null) return false;
            for (int i = 0; i < visible.Count; i++) if (visible[i] == ability) return true;
            return false;
        }
