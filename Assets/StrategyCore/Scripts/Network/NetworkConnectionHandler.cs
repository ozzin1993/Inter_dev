    ...
    public class NetworkConnectionHandler : MonoBehaviour
    {
        ...
        private void ClientConnected(ulong clientId)
        {
            ...
            if (SlotManager.instance.gameStarted == GameState.Menu)
            {
                ...
                if (UIManagerMenu.instance != null)
                {
            ...
            if (NetworkManager.Singleton.IsServer)
            {
                ...
                if (connectedSlot != -1)
                {
                    ...
                    // [Interflow fix 2026-06-26] null-guard: на сервере соответствующего UI-синглтона может не быть.
                    if (SlotManager.instance.gameStarted == GameState.Menu) { if (UIManagerMenu.instance != null) UIManagerMenu.instance.AddChatServerMsg(msg); }
                    else if (UIManager.instance != null) UIManager.instance.AddChatServerMsg(msg);
                }
            }
        }
        ...
        private void ClientDisconnected(ulong clientId)
        {
            ...
            else
            {
                ...
                if (SlotManager.instance.gameStarted == GameState.Menu)
                {
                    // Refresh lobby
                    // [Interflow fix 2026-06-26] null-guard: меню-UI может отсутствовать на сервере.
                    if (UIManagerMenu.instance != null) UIManagerMenu.instance.FillPlayerList();
                }
