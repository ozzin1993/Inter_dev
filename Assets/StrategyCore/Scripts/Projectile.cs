    ...
    public class Projectile : MonoBehaviour
    {
        ...
        void Update()
        {
            ...
            if (!target && followTarget)
            {
                ...
                if (impactSound != null && impactSound.groundHitClips != null && impactSound.groundHitClips.Length > 0) SoundFXManager.instance.PlaySoundClip(impactSound.groundHitClips, this.transform, 1, SoundFXManager.instance.fxGroup);
            ...
            if (posFactor >= 1 || (targetPosition - transform.position).sqrMagnitude < radiusSq)
            {
                ...
                if (followTarget)
                {
                    ...
                    if (impactSound != null && impactSound.weaponSound[target.armorType.index].audioClips != null) SoundFXManager.instance.PlaySoundClip(impactSound.weaponSound[target.armorType.index].audioClips, this.transform, 1, SoundFXManager.instance.fxGroup);
                }
                else
                {
                    // Play ground hit Sound - Attack sound, target and audio clips must be defined
