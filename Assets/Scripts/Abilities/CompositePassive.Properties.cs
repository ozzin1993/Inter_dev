...
namespace StrategyCore
{
    ...
    public class PassiveControlImmunityBlock
    {
        ...
        [Tooltip("Когда блок работает. «Пока здоровья меньше доли» — иммунитет включается и выключается " +
                 "сам, пока умение открыто. Условие считается штатным тиком 0,1 с, поэтому включение " +
                 "и выключение идут с задержкой до одного тика.")]
        public PassiveBlockCondition carrierCondition = PassiveBlockCondition.Always;

        [Tooltip("Порог для условия «пока здоровья меньше доли»: 0.2 — блок работает, пока здоровья " +
                 "СТРОГО меньше пятой части максимума. 0 — условия по здоровью нет.")]
        [Range(0f, 1f)]
        public float carrierHpBelow;

        [Tooltip("Длящийся визуал: состояние, которое висит на носителе, пока условие выполнено. " +
    ...
    public class PassiveResistancesBlock
    {
        ...
        [Tooltip("Строки «категория — доля». Из всех источников на юните (пассивки, бафы) действует ОДНО значение: " +
                 ...
                 "и уже висящие наложения этой категории СНИМАЮТСЯ в момент включения блока.")]
        public ResistanceEntry[] entries;

        [Tooltip("Когда блок работает. «Пока здоровья меньше доли» — сопротивления включаются и выключаются " +
                 "сами, пока умение открыто. Условие считается штатным тиком 0,1 с, поэтому включение " +
                 "и выключение идут с задержкой до одного тика.")]
        public PassiveBlockCondition carrierCondition = PassiveBlockCondition.Always;

        [Tooltip("Порог для условия «пока здоровья меньше доли»: 0.3 — блок работает, пока здоровья " +
                 "СТРОГО меньше трети максимума. 0 — условия по здоровью нет.")]
        [Range(0f, 1f)]
        public float carrierHpBelow;
    ...
    public partial class CompositePassive
    {
        ...
        void ApplyControlImmunity(Unit unit, Carrier c)
        {
            ...
            // [Interflow 2026-09-17, решение Artsiom 41] Условие по здоровью посчитано до вызова —
            // здесь только читается. Метод зовут ДВА входа: открытие умения и пересечение порога
            // в тике; второго пути выдачи блока в проекте нет (правило 5).
            if (!c.controlImmunityGate)
            {
                if (InterflowDebug.FullOn) LogBlockSkipped(2, unit, "выдача", "условие по здоровью не выполнено");
                return;
            }

            // Повторная выдача без смены состояния запрещена: счётчик источников иммунитета
            // рос бы каждым тиком, и снятие уже никогда не сработало бы.
        ...
        void ApplyResistances(Unit unit, Carrier c)
        {
            ...
            // [Interflow 2026-09-17, решение Artsiom 41] См. блок 2: условие читается, а не считается.
            if (!c.resistancesGate)
            {
                if (InterflowDebug.FullOn) LogBlockSkipped(3, unit, "выдача", "условие по здоровью не выполнено");
                return;
            }

            if (c.resistances) return;   // повторная выдача без смены состояния запрещена
            ...
            // [Interflow 2026-09-17, решение Artsiom 42] Снятие по категории: сопротивление отбивает
            // только НОВЫЕ наложения, поэтому «иммунитет снимает уже висящее замедление» без этого
            // вызова не выражается ничем. Только сервер — открытие умения идёт на обоих пирах,
            // а снятие состояния меняет состояние мира (правило 6).
            if (!IsClientPeer) DispelFullyResisted(unit, holder);
