...
namespace StrategyCore
{
    ...
    public class StatusIconCatalog : ScriptableObject
    {
        ...
        // [Interflow 2026-09-18, решение Artsiom 50] Служебное состояние «в полёте» — отметка на время
        // полёта отброшенной цели (блок 15, travelSeconds). Лежит здесь, рядом с контролем, по той же
        // причине: ссылок в коде не держим (правило 3), второй справочник ради одного поля не заводим
        // (правило 5). Накладывает воронка Unit.InFlightStart.
        [Tooltip("Состояние, которое накладывает воронка полёта Unit.InFlightStart (блок 15 «Отбросить цели»). " +
                 "У ассета обязана быть категория «В полёте (служебная)» — именно по ней выводится флаг " +
                 "«юнит летит» и именно она проводит наложение мимо сопротивлений и иммунитета к контролю. " +
                 "Запрет активных действий на время полёта задают галки «Обезоруживает» и «Накладывает немоту». " +
                 "Иконки и VFX быть НЕ должно: состояние служебное, клиенту оно не уезжает")]
        public Effector inFlightEffector;
