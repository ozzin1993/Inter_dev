...
namespace StrategyCore
{
    ...
    public class DeathEffects : MonoBehaviour
    {
        ...
        [Tooltip("Технология, без которой посмертные эффекты НЕ срабатывают (гейт специализации, например «Финальное искупление»). " +
                 "Пусто — эффекты работают всегда. Добавлено 2026-07-24: компонент, в отличие от способности, " +
                 "не умеет включаться сам по Required Tech.")]
        [SerializeField] private Technology requiredTech;
        ...
        private void OnUnitDie(Unit dies, int killerPlayer, Unit killerUnit, bool rewards)
        {
            ...
            // Гейт специализации: технология не изучена — посмертных эффектов нет
            // (обёртка InterflowCombat гарантирует, что нейтралы и незарегистрированные техи не уронят вызов)
            if (requiredTech != null && !InterflowCombat.IsTechUnlockedSafe(requiredTech, unit.owner)) return;

            InterflowDebug.Event("ПОСМЕРТНЫЙ ЭФФЕКТ у " + InterflowDebug.Name(unit) + ": сработал" +
                                 (requiredTech != null ? " (технология «" + requiredTech.name + "» изучена)" : ""));
