...
namespace StrategyCore
{
    ...
    // ============================= ЭФФЕКТЫ ОТКРЫТИЯ (узлы дерева технологий) ==
    // Что открывает узел — задаётся НА САМОМ УЗЛЕ (TechNode.unlock*), а не отдельным списком правил
    // с условиями (решение Artsiom 2026-07-24). Условие одно и всегда одинаковое: узел куплен.
    // Уровень ГЗ условием НЕ является (он остаётся только в статах/облике замка и генерации душ).
    // Применяет сервер — MatchManager.RecomputeUnlockedContent (идемпотентный пересчёт из источника).
    ...
    /// <summary>Умение ГЗ, открываемое узлом дерева: само умение + номер его ячейки в панели (0..2).</summary>
    [Serializable]
    public class AbilityUnlock
    {
        [Tooltip("Умение главного здания, которое открывает этот узел.")]
        public Ability ability;
        [Tooltip("Номер ячейки в панели умений ГЗ: 0, 1 или 2 (панель — один ряд из трёх). " +
                 "Ячейка закреплена: пока умение не открыто, она пустая; открытое умение всегда на своём месте. " +
                 "Две записи на одну ячейку — ошибка настройки (ловит валидатор Interflow Editor).")]
        [Range(0, 2)] public int slot;
    }
    ...
    public class TechNode
    {
        ...
        [Header("Что открывает узел")]
        [Tooltip("Юниты, которые узел добавляет в ДОСТУПНЫЕ для волны (игрок сможет помечать их [Авто]/[Разовый]). " +
                 "Пусто — узел юнитов не открывает.")]
        public Unit[] unlockUnits;
        [Tooltip("Умения главного здания, которые узел добавляет в панель умений (умение + номер ячейки 0..2). " +
                 "Пусто — узел умений не открывает.")]
        public AbilityUnlock[] unlockAbilities;

        [Header("Подмена префабов")]
        [Tooltip("Подмена префабов юнитов волны (from→to) — действует на будущие спавны. Пусто — не подменять.")]
        public UnitSwap[] unitSwaps;
        [Tooltip("Подмена префабов башен по типу точки — применяется и к уже стоящим башням. Пусто — не подменять.")]
    ...
    {
        [Tooltip("Узел самого большого выбора. Что он открывает — задаётся на самом узле (блоки «Что открывает узел» / «Подмена префабов»); герой — полем heroPrefab ниже.")]
    ...
    public class FactionConfig : ScriptableObject
    {
        ...
        [Tooltip("Единый список юнитов волны: каждая запись — юнит + роль (Базовый/Доступный) + число копий. " +
                 ...
                 "(технологии открывают доп. юнитов поверх — полем TechNode.unlockUnits, у них по 1 копии).")]
        public WaveUnitEntry[] waveUnits;

        [Tooltip("Базовый доход золота за волну по УРОВНЮ главного здания (ГЗ). Начисляется одним событием " +
                 "в момент призыва волны — другого источника дохода нет. ИНДЕКС = УРОВЕНЬ ГЗ: элемент 0 — " +
                 "уровень 0 (начало матча), элемент 1 — уровень 1, и так далее. ВНИМАНИЕ: у таблицы душ " +
                 "(soulsPerMinuteByMbLevel) индексация ДРУГАЯ — там элемент 0 = уровень 1; не перепутай. " +
                 "Уровень выше последней записи клампится на последнюю (короткая таблица замирает на ней). " +
                 "Пустая таблица — доход 0. Таблица ОБЯЗАНА быть неубывающей: предел автопризыва проверяется " +
                 "в момент пометки, и падение дохода к моменту волны сделало бы списание больше начисления. " +
                 "[БАЛАНС — Влад]")]
        public int[] waveIncomeByLevel;

        [Header("Способности центральной таблицы")]
        [Tooltip("СТАРТОВЫЕ умения главного здания — те, что доступны с начала матча. Порядок = номер ячейки " +
                 "панели: элемент 0 → левая ячейка, 1 → средняя, 2 → правая (панель — один ряд из трёх). " +
                 "Больше трёх записей класть нельзя — лишние в панель не поместятся. Умения, которые открываются " +
                 "по ходу матча, сюда НЕ добавляются: они задаются на узлах дерева технологий (TechNode.unlockAbilities). " +
                 "На старте MatchManager пропишет все умения расы в abilities[] объекта-кастера команды " +
                 "(невидимый юнит сцены, ссылка TeamWaveConfig.abilityCaster). Иконка — из Ability.icon[0]. " +
                 "Поддержаны Active-способности.")]
        ...
        [Tooltip("Префаб героя расы (Unit). Призывается кнопкой вне волны; его abilities[] показываются в таблице умений героя. " +
                 "Умения открываются только технологией (Ability.requiredTech; открытие по уровню снесено Б8); опыт/уровни — компонент LevelingUnit на префабе. " +
