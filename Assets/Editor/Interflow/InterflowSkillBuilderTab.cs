...
namespace StrategyCore
{
    ...
    public static partial class InterflowSkillBuilderTab
    {
        ...
        class BlockDesc
        {
            // НОМЕР БЛОКА — он же то число, которым блок зовут в документах, в сообщениях валидатора
            // и в справочнике полей для ГД. Порядку исполнения он равен НЕ всегда: решением Artsiom 48
            // от 18.09.2026 блок 18 исполняется ПЕРВЫМ, до цикла по целям, а номер за ним остался
            // (переномеровать значило бы переписать все ссылки на блоки). Фактическое место — в подсказке.
            public int order;            // номер блока
            ...
            public bool projectileOnly;  // чип показывать только при доставке снарядом (блок 22)
        }
        ...
        static readonly BlockDesc[] BLOCKS =
        {
            ...
            // [Interflow 2026-09-18, решение Artsiom 53] perTarget = false: с флагом «облик на себя»
            // блок применяется ОДИН раз за каст к кастеру, а цели не трогаются вовсе. Признак в редакторе
            // описывает ту форму, ради которой блок правился; при выключённом флаге блок по-прежнему
            // идёт по каждой цели — это сказано в подсказке, чтобы описание и поведение не разъехались.
            new BlockDesc { order = 12, field = "morph",           title = "Подмена облика",           kind = StepKind.State, perTarget = false,
                            hint = "принимает вид другого юнита на время; статы меняются только заданными пассивными эффектами. " +
                                   "С флагом «облик на себя» — ОДИН раз за каст к кастеру (цели не трогаются), " +
                                   "без флага — по каждой цели. Флаг «способ атаки» копирует у заготовки ближний бой, тип и дальность атаки и снаряд" },
            ...
            new BlockDesc { order = 15, field = "knockback",       title = "Отбросить цели",           kind = StepKind.Event, perTarget = true,
                            hint = "идёт последним по каждой цели: всё позиционное (вторичные цели, зона, визуал) считается до сдвига. " +
                                   "Отброс достаётся ВСЕМ, кого задело умение — своего отбора у блока нет. " +
                                   "Время полёта — только показ: геймплейно цель оказывается в конечной точке сразу и на это время помечается «в полёте»" },
            ...
            new BlockDesc { order = 17, field = "groundZone",      title = "Зона на земле",            kind = StepKind.Event, perTarget = false,
                            hint = "исполняется один раз за каст; урон и эффекты живут на префабе зоны. " +
                                   "Режим «шлейф по пути» выкладывает зоны по прямой от старта до приземления блока 18 — " +
                                   "во времени, за время полёта; без блока 18 не работает" },
            new BlockDesc { order = 18, field = "casterMove",      title = "Перемещение кастера",      kind = StepKind.Event, perTarget = false,
                            hint = "кастер переносится в точку приложения — телепорт, рывок, прыжок. " +
                                   "Идёт ПЕРВЫМ, до блоков по целям: отброс и рывок считаются уже от точки приземления. " +
                                   "Время полёта и высота дуги — только показ у клиента; геймплейно перенос мгновенный" },
            ...
            new BlockDesc { order = 20, field = "gaugeBlock",      title = "Шкала",                    kind = StepKind.Event, perTarget = false,
                            hint = "каст требует полную шкалу (gauge) и тратит её в ноль" },
            new BlockDesc { order = 21, field = "castConditions",  title = "Условия каста",            kind = StepKind.Event, perTarget = false,
                            hint = "гейт ПЕРЕД кастом: облик кастера, «есть цель в области», порог здоровья; отказ приходит игроку текстом, у кнопки гаснет иконка" },
            new BlockDesc { order = 22, field = "projectileImpact", title = "Прилёт снаряда",           kind = StepKind.Event, perTarget = false, projectileOnly = true,
                            hint = "в точке, куда прилетел снаряд ЭТОГО умения, исполняется другое умение — от лица стрелка, один раз на каждый прилёт (в том числе на каждый рикошет); работает только при доставке снарядом" },
        };

        /// <summary>
        /// Блоки, которые полоса чипов показывает при текущих настройках умения. Блок 22 виден только
        /// при доставке снарядом: без снаряда прилетать нечему. Уже ВКЛЮЧЁННЫЙ блок при этом не пропадает
        /// из редактора — его карточка строится по факту включения (AddBlockCards) и выключается там же,
        /// а валидатор кричит правилом Р13.
        /// </summary>
        static IEnumerable<BlockDesc> VisibleBlocks()
        {
            foreach (var b in BLOCKS)
            {
                if (b.projectileOnly && (selected == null || selected.delivery != SkillDelivery.Projectile)) continue;
                yield return b;
            }
        }
        ...
        static readonly (string key, string field)[] ISSUE_ROUTES =
        {
            ...
            ("услов",       "castConditions"),  // ВЫШЕ «облик» и «зон»: сообщения блока 21 говорят и о том, и о другом
            ("прилёт",      "projectileImpact"),// ВЫШЕ «урон» и «снаряд»: сообщения блока 22 говорят и о снаряде, и о вложенном умении
            ("баф",         "buff"),
            ("ауры",        "buff"),
            ("взрыв",       "buff"),
            ("щит",         "shield"),
            ...
            ("оттолкн",     "knockback"),
            ("нокбэк",      "knockback"),
            // [Interflow 2026-09-18] «шлейф» и «прыжок» — ВЫШЕ «зон» и «перемещен» соответственно
            // не нужно (оба ведут в тот же блок), но выше «рывок» обязательно: ключ «рывок» занят
            // блоком 2 (рывок ЦЕЛИ к кастеру), и сообщения про рывок КАСТЕРА он бы перехватил.
            // Поэтому сообщения блока 18 пишутся словами «перемещение» и «прыжок», а не «рывок».
            ("шлейф",       "groundZone"),
            ("прыжок",      "casterMove"),
            ("рывок",       "pull"),
            ("притяг",      "pull"),
            ...
            ("урон",        "damage"),
            ("урона",       "damage"),
            // [Interflow 2026-09-17] «визуал» — САМЫМ ПОСЛЕДНИМ, после всех ключей блоков: сообщения
            // правил Р24-Р30 говорят и о визуале, и о своём блоке, и более общий ключ перехватил бы
            // их у частного (так уже случалось с ключом «урон» и аурой бафа).
            ("визуал",      null),
        };
